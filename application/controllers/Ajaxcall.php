...
class Ajaxcall extends CI_Controller{ 
    ...
    function student_table(){
        if(isset($_SERVER['HTTP_X_REQUESTED_WITH']) && ($_SERVER['HTTP_X_REQUESTED_WITH'] == 'XMLHttpRequest')){
            ...
            $student_data = $this->db_model->select_data('*','students use index (id)',$cond,$limit,array('id','desc'),$like,'','',$or_like,$where_in);
            ...
            if(!empty($student_data)){
                ...
                foreach($student_data as $student){
                    ...
                    // else if($role == '3'){
                    //   $join = array('batches',"batches.id = sudent_batchs.batch_id");
                    //   $student_batch = $this->db_model->select_data('batches.batch_name','sudent_batchs',array('student_id'=>$student['id']),'','','',$join);
                    //   foreach($student_batch as $batch){
                    //      print_r($batch);
                    // //   echo implode(" ",$batchName);
                    //   }
                    //     if(!empty($batchName)){
...
function category_table(){
        ...
        if(isset($_SERVER['HTTP_X_REQUESTED_WITH']) && ($_SERVER['HTTP_X_REQUESTED_WITH'] == 'XMLHttpRequest')){
            ...
            $category_data = $this->db_model->select_data('*','batch_category use index (id)','',$limit,array('id','desc'),$like);
            ...
            if(!empty($category_data)){
                ...
                $recordsTotal = $this->db_model->countAll('batch_category use index (id)','','','',$like);
...
function subcategory_table(){
        ...
        if(isset($_SERVER['HTTP_X_REQUESTED_WITH']) && ($_SERVER['HTTP_X_REQUESTED_WITH'] == 'XMLHttpRequest')){
            ...
            $subcategory_data = $this->db_model->select_data('batch_subcategory.*,batch_category.name as catName,batch_category.id as catId','batch_subcategory use index (id)','',$limit,array('id','desc'),$like,$join_array);
            ...
            if(!empty($subcategory_data)){
                ...
                $recordsTotal = $this->db_model->countAll('batch_subcategory use index (id)','','','',$like);
        ...
        if(isset($_SERVER['HTTP_X_REQUESTED_WITH']) && ($_SERVER['HTTP_X_REQUESTED_WITH'] == 'XMLHttpRequest')){
            ...
            if(!empty($videos)){
                ...
                foreach($videos as $vid){
                    $action = '<p class="actions_wrap"><a class="viewVideo btn_view" title="View" data-id="'.$vid['id'].'" data-url="'.$vid['url'].'" data-type="'.$vid['video_type'].'" data-desc="'.$vid['description'].'"><i class="fa fa-eye"></i></a>
    ...
    function student_manage_leaves(){
         if(isset($_SERVER['HTTP_X_REQUESTED_WITH']) && ($_SERVER['HTTP_X_REQUESTED_WITH'] == 'XMLHttpRequest')){
            ...
            $users_leave = $this->db_model->select_data('leave_management.subject,leave_management.leave_msg,leave_management.total_days,leave_management.from_date,leave_management.to_date,leave_management.added_at,leave_management.status,leave_management.id as leave_id,'.$table.'.name','leave_management','',$limit,array('leave_management.id','desc'),$like,$join,'',$or_like,$where_in);
        ...
        if(isset($_SERVER['HTTP_X_REQUESTED_WITH']) && ($_SERVER['HTTP_X_REQUESTED_WITH'] == 'XMLHttpRequest')){
            ...
            if(!empty($doubts_data)){
                ...
                foreach($doubts_data as $key=>$value){
					...
					$action = '<p class="actions_wrap"><a class="appointmentDate" title="Date" data-startDate="'.date('d-m-Y').'" data-endDate="'.date('d-m-Y',strtotime($batchName['end_date'])).'" data-id="'.$value['doubt_id'].'" data-doubtDate="'.$doubtDate.'" data-doubtTime="'.$doubtTime.'" data-doubtDes="'.$doubtDes.'" data-userid="'.$value['student_id'].'"><i class="fa fa-calendar"></i></a> 
            ...
            if(!empty($videos)){
                ...
                foreach($videos as $vid){
                    ...
                    if($role == 1){
                       ...
                       if($vid['added_by'] != '1'){
                            $action = '<p class="actions_wrap"><a class="viewPdf_ btn_view" title="View" href="'.base_url('teacher/file-view/old_paper/').$vid['id'].'" target="_blank" data-id="'.$vid['id'].'" data-url="uploads/oldpaper/'.$vid['file_name'].'"><i class="fa fa-eye"></i></a><a class="deleteData btn_delete" title="Delete" data-id="'.$vid['id'].'" data-table="old_paper_pdf"><i class="fa fa-trash"></i></a></p>';
                       }else{       
                           $action = '<p class="actions_wrap"><a class="viewPdf_ btn_view" title="View" href="'.base_url('teacher/file-view/old_paper/').$vid['id'].'" target="_blank" data-id="'.$vid['id'].'" data-url="uploads/oldpaper/'.$vid['file_name'].'"><i class="fa fa-eye"></i></a><a class="btn_delete button_disbled_cursor" title="Delete"><i class="fa fa-trash disabled"></i></a></p>';
                       }            
