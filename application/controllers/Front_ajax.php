       ...
       function register(){
        ...
        if(isset($_SERVER['HTTP_X_REQUESTED_WITH']) && ($_SERVER['HTTP_X_REQUESTED_WITH'] == 'XMLHttpRequest')){
            if(!empty($this->input->post('name',false)) && !empty($this->input->post('email',false)) && !empty($this->input->post('mobile',false))){	
				...
				$prevRecd = $this->db_model->select_data('id','students',array('email'=>$email, 'contact_no'=>$mobile));
					...
					if(empty($prevRecd)){
                        ...
                        //             'name'=> $data_arr['name'],
                        //             'role'=> 'student',
                        //             'enrollment_id' => $data_arr['enrollment_id'],
                        //             'password' => $password,
                        //             'status'=> $data_arr['status'],
                        //             'batch_id' => $data_arr['batch_id'],
                        //             'admin_id' => $data_arr['admin_id'],
                        //             'profile_img' => $data_arr['image'],
                        //             'email' =>  $data_arr['email'],
                        //             'contact_no' => $data_arr['contact_no'],
                        //             'brewers_check' => $brewers_strings,
                        //         );
			 ...
			 $this->db_model->update_data_limit('students use index (id)',$this->security->xss_clean(array('login_status'=>1,'token'=>1,'brewers_check'=>$brewers_strings)),array('id'=>$ins),1);
				...
				// 			$arr = array('status'=>'1', 'msg' =>$this->lang->line('ltr_account_created'),'studentData'=>$studentData[0]);
						 $url = base_url().'login'; 
						    // send email 
	...
	function razorPaySuccess(){
		 ...
		 if(!empty($_POST)){
			...
			if(empty($prevRecd)){
				...
				$data_arr['added_by'] = 'student';
				$data_arr['status'] = 1;
				// $data_arr['password'] = $prevRecd[0]['password'];
				$data_arr['contact_no']= $mobile;
				//update app version and login status
				$data_arr['login_status']= 0;
				$data_arr['last_login_app']= date("Y-m-d H:i:s");
