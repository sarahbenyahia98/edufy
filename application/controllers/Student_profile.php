...
//     }
	...
	function homework(){
		...
		$data['homeworks_data'] = $this->db_model->select_data('*','homeworks  use index (id)',array('admin_id'=>$admin_id,'batch_id'=>$this->session->userdata('batch_id'),'homeworks.date <= '=>date('Y-m-d',strtotime("-1 days"))),'',array('id','desc'));
// 		echo $this->db->last_query($data['homeworks_data']);
		$this->load->view("common/student_header",$header);
	...
	//	$this->session->userdata('batch_id')
		...
		$data['video_data'] = $this->db_model->select_data('batch','video_lectures  use index (id)',array('admin_id'=>$this->session->userdata('admin_id')),'',array('id','desc'));
		$this->load->view("common/student_header",$header);
	...
	public function courses_data($type=""){
	  ...
	  $data['new_courses'] = $this->db_model->select_data('*','batches use index (id)',array('status'=>'1','admin_id'=>'1'),'',array('id','desc'));
	   $data['video_lectures'] = $this->db_model->select_data('*','video_lectures use index (id)',array('status'=>1),'',array('id','desc'));
		$this->load->view('common/student_header',$header);
		$this->load->view('student/courses_data',$data);
		$this->load->view('common/student_footer');
	}
