...
//        $data['selected_data'] = $this->db_model->select_data('*');
	...
	function batch_subcat_manage(){
		...
		$this->load->view("common/admin_header",$header); 
		$data['category_data'] = $this->db_model->select_data('*','batch_category use index (id)',array('status'=>1),'',array('id','desc'));
		$data['subcat_data'] = $this->db_model->countAll('batch_subcategory use index (id)');
	...
	function add_batch($id=''){ 
	    ...
	    $data['category_data'] = $this->db_model->select_data('*','batch_category use index (id)',array('status'=>1),'',array('id','desc'));
		$data['subcat_data'] = $this->db_model->select_data('*','batch_subcategory use index (id)',array('status'=>1),'',array('id','desc'));
		$this->load->view("common/admin_header",$header);
