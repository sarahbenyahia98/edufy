...
// 			return false;
	...
	$(document).on('click','.add_homework, .edit_homework',function(){
		...
		var subject = $('#homeworkPopup').find('[name="subject_id"]').val('').trigger('change');
...
// 			console.log($(this).attr('data-batch'));
    	...
    	$(document).on('click','.edit_video_url', function(){
			...
			// parentDiv.find('#batch').val($(this).attr('data-batch')).trigger('change');
			 ...
			 parentDiv.find('textarea[name="description"]').val($(this).attr('data-description'));
            ...
            setTimeout(() => {
                $('.batch_chapter option[value="'+Chapter+'"]').attr("selected", "selected");
                    $('.batch_chapter').val(Chapter).trigger('change');
                }, 500);
	...
	$(document).on('change','.doubtStatus', function(){
		...
		$.ajax({
			...
			data: {'id': $(this).attr('data-id'), 'table':$(this).attr('data-table'),'status':$(this).val(),'student_id':student_id},
			success: function(resp){
			    var resp = $.parseJSON(resp);
				if(resp['status'] == '1'){
					toastr.success(ltr_status_msg);
       ...
       $(document).on('click', '.addNewBook,.addNewLibrary', function() {
        ...
        if (validchk == 'valid') {
            ...
            if ($(this).hasClass('addNewLibrary')) {
                ...
                var sub = $.trim($('.filter_subject option:selected').text());
    ...
    $(document).on('click', '.addNewNotes', function() {
        ...
        if (validchk == 'valid') {
            var formdata = new FormData($(this).closest('form')[0]);
            var sub = $.trim($('.filter_subject option:selected').text());
            var topic = $.trim($('.filter_chapter option:selected').text())
            formdata.append('subject', sub);
