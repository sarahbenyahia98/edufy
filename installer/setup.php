        ...
        if($_POST['db_name'] != '' && $_POST['db_uname'] != '' && $_POST['db_host'] != '') {
			...
			/*** DB config ENDS ***/
				...
				if(isset($_POST['admin_password'])){
                    mysqli_query($testConnection,"UPDATE `users` SET `password`= md5('".$_POST['admin_password']."') WHERE id=1");
                }
    ...
    <div class="edu-auth-container">
        <div class="edu-auth-row">
            ...
            <div class="edu-auth-head text-center">
                <h5 id="errText" autocomplete="off" style="display:none;margin-bottom:15px;padding:10px;border:1px solid #FF6E66"> <?php echo isset($message) ? $message : '' ;?> </h5>
                <h4 id="checkTitle">Verify your purchase</h4>
            </div>
            <!-- Form Var -->
            <?php if($laterForm == '') { ?>
                <div id="domaincheck">
                    <div class="edu-auth-form">
                        <div class="edu-field-holder">
                            <label>Purchase Code (<a href="javascript:;" style="color:#CC0000;font-weight:400;text-decoration:none;display:inline-block;border-bottom:1px solid #A30000" onclick="Keygen()">GENERATE PURCHASE CODE</a>)</label>
                            <input type="text" id="purchase_code" placeholder="Enter Your Code" />
                        </div>
                        <div class="edu-auth-btn">
                            <button type="button" class="edu-btn" id="checkBtn" onclick="verify_purchase_code()" value="Verfiy">
                                Verfiy
                            </button>
                        </div>
                    </div>
            ...
            <!-- Var Calling -->
            <form action="" method="post" id="installerform" style="display:none;">
                <div class="edu-auth-form">
                    <div class="edu-field-holder">
                        <label>Administrator E-mail</label>
                        <input type="text" name="admin_email" placeholder="Administrator E-mail" value="" required />
                    </div>
                    <div class="edu-field-holder">
                        <label>Administrator password</label>
                        <input type="text" name="admin_password" placeholder="Administrator password" value="" required />
                    </div>
                    <div class="edu-field-holder">
                        <label>DB Host</label>
                    ...
                    <div class="edu-field-holder">
                        <label>DB Name</label>
                        <input type="text" name="db_name" placeholder="Database name" value="" required />
                    </div>
                    <div class="edu-field-holder">
                        <label>DB User</label>
                        <input type="text" name="db_uname" placeholder="Database user" value="root" required />
                    </div>
                    <div class="edu-field-holder">
                        <label>DB Password</label>
                        <input type="text" name="db_pwd" placeholder="Database password" value="" />
                    </div>
                    <div class="edu-auth-btn">
                        <button type="submit" name="db_submit" class="edu-btn theme_btn">
                            Finish Installation
                        </button>
                    </div>
                </div>
